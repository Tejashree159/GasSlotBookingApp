...
Mongoose.prototype.setDriver = function setDriver(driver) {
  const mongooose = this instanceof Mongoose ? this : mongoose;

  if (mongooose.__driver === driver) {
    return mongooose;
  }

  const openConnection = mongooose.connections && mongooose.connections.find(conn => conn.readyState !== STATES.disconnected);
  if (openConnection) {
  ...
  mongooose.__driver = driver;

  const Connection = driver.Connection;
  const oldDefaultConnection = mongooose.connections[0];
  mongooose.connections = [new Connection(mongooose)];
  mongooose.connections[0].models = mongooose.models;
  ...
  for (const model of Object.values(mongooose.models)) {
    if (model.db !== oldDefaultConnection) {
      continue;
    }
    model.$__updateConnection(mongooose.connections[0]);
  }
...
Mongoose.prototype.set = function(key, value) {
  ...
  for (const [optionKey, optionValue] of Object.entries(options)) {
    ...
    mongooose.options[optionKey] = optionValue;

    if (optionKey === 'objectIdGetter') {
      if (optionValue) {
        Object.defineProperty(mongooose.Types.ObjectId.prototype, '_id', {
          enumerable: false,
          configurable: true,
          get: function() {
            return this;
        ...
        delete mongooose.Types.ObjectId.prototype._id;
      }
    } else if (optionKey === 'transactionAsyncLocalStorage') {
      if (optionValue && !mongooose.transactionAsyncLocalStorage) {
        mongooose.transactionAsyncLocalStorage = new AsyncLocalStorage();
      } else if (!optionValue && mongooose.transactionAsyncLocalStorage) {
        delete mongooose.transactionAsyncLocalStorage;
      }
    }
  }
...
Mongoose.prototype.createConnection = function(uri, options) {
  ...
  const Connection = mongooose.__driver.Connection;
  const conn = new Connection(mongooose);
  mongooose.connections.push(conn);
  mongooose.nextConnectionId++;
  mongooose.events.emit('createConnection', conn);
...
Mongoose.prototype.connect = async function connect(uri, options) {
  ...
  const mongooose = this instanceof Mongoose ? this : mongoose;
  const conn = mongooose.connection;

  return conn.openUri(uri, options).then(() => mongooose);
};
...
Mongoose.prototype.disconnect = async function disconnect() {
  ...
  const remaining = mongooose.connections.length;
  if (remaining <= 0) {
    return;
  }
  await Promise.all(mongooose.connections.map(conn => conn.close()));
};
...
Mongoose.prototype.startSession = function() {
  ...
  return mongooose.connection.startSession.apply(mongooose.connection, arguments);
};
...
Mongoose.prototype.pluralize = function(fn) {
  ...
  if (arguments.length > 0) {
    mongooose._pluralize = fn;
  }
  return mongooose._pluralize;
};
...
Mongoose.prototype.model = function(name, schema, collection, options) {
  ...
  if (arguments.length === 1) {
    const model = mongooose.models[name];
    if (!model) {
  ...
  if (schema) {
    if (mongooose.get('cloneSchemas')) {
      schema = schema.clone();
    }
    mongooose._applyPlugins(schema);
  }

  // connection.model() may be passing a different schema for
  // an existing model name. in this case don't read from cache.
  const overwriteModels = mongooose.options.hasOwnProperty('overwriteModels') ?
    mongooose.options.overwriteModels :
    options.overwriteModels;
  //if (mongooose.models.hasOwnProperty(name) && options.cache !== false && overwriteModels !== true) {
    //if (originalSchema &&
      //  originalSchema.instanceOfSchema &&
        //originalSchema !== mongooose.models[name].schema) {
      //throw new mongooose.Error.OverwriteModelError(name);
    //}
    if (collection && collection !== mongooose.models[name].collection.name) {
      // subclass current model with alternate collection
      const model = mongooose.models[name];
      schema = model.prototype.schema;
      const sub = model.__subclass(mongooose.connection, schema, collection);
      // do not cache the sub model
      return sub;
    }
    return mongooose.models[name];
  }
  

  const model = mongooose._model(name, schema, collection, options);
  mongooose.connection.models[name] = model;
  mongooose.models[name] = model;
...
Mongoose.prototype._model = function(name, schema, collection, options) {
  ...
  if (typeof name === 'function') {
    ...
    if (!(model.prototype instanceof Model)) {
      throw new mongooose.Error('The provided class ' + name + ' must extend Model');
    }
  }
  ...
  if (schema == null || !('pluralization' in schema.options)) {
    schema.options.pluralization = mongooose.options.pluralization;
  }

  if (!collection) {
    collection = schema.get('collection') ||
      utils.toCollectionName(name, mongooose.pluralize());
  }

  const connection = options.connection || mongooose.connection;
  model = mongooose.Model.compile(model || name, schema, collection, connection, mongooose);
...
Mongoose.prototype.deleteModel = function(name) {
  ...
  mongooose.connection.deleteModel(name);
  delete mongooose.models[name];
  return mongooose;
};
...
Mongoose.prototype.modelNames = function() {
  ...
  const names = Object.keys(mongooose.models);
  return names;
};
...
Mongoose.prototype._applyPlugins = function(schema, options) {
  ...
  options = options || {};
  options.applyPluginsToDiscriminators = mongooose.options && mongooose.options.applyPluginsToDiscriminators || false;
  options.applyPluginsToChildSchemas = typeof (mongooose.options && mongooose.options.applyPluginsToChildSchemas) === 'boolean' ?
    mongooose.options.applyPluginsToChildSchemas :
    true;
  applyPlugins(schema, mongooose.plugins, options, '$globalPluginsApplied');
};
...
Mongoose.prototype.plugin = function(fn, opts) {
  ...
  mongooose.plugins.push([fn, opts]);
  return mongooose;
};
...
Mongoose.prototype.isValidObjectId = function(v) {
  const mongooose = this instanceof Mongoose ? this : mongoose;
  return mongooose.Types.ObjectId.isValid(v);
};
...
Mongoose.prototype.syncIndexes = function(options) {
  const mongooose = this instanceof Mongoose ? this : mongoose;
  return mongooose.connection.syncIndexes(options);
};
